...
const LowerLeftCards = ({ cardsData }: CardsProps) => {
  const iconClassName = "h-[50px] w-[50px] text-[#2CA58D] dark:text-[#cfe2e2]";

  type CardTitle = "Monthly Attendance";

  const iconMap: Record<CardTitle, JSX.Element> = {
    "Monthly Attendance": <BsCalendarRangeFill className={iconClassName} />,
  };
