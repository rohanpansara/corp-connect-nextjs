...
const RightSideCards = ({ data }: RightSideCardsProps) => {
  ...
  const activeCard = active === 'today' ? todayAttendanceCard : weeklyAttendanceCard
        ...
        <Card className='h-full flex flex-col p-2 border-none bg-cardBackgroundColor rounded-md'>
          <CardHeader className='p-2'>
            <div className='flex justify-between items-center w-full'>
              <div className='w-1/2 text-xs text-cardTextColor font-semibold'>{activeCard?.title}</div>
              ...
              <div className='flex justify-center items-center flex-col'>
                <span
                  className={`font-bold text-cardTextColor ${
                    activeCard?.remainingValue < 0 ? `text-[40px]` : `text-[25px]`
                  }`}
                >
                  {activeCard?.value}
                </span>
