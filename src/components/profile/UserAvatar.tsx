...
const UserAvatar = ({ userHeaderData }: UserAvatarProps) => {
  const profileImageUrl = userHeaderData?.id ? `/images/users/${userHeaderData.id}.png` : undefined
    ...
    <div className='relative group cursor-pointer'>
      <Avatar className='h-[120px] w-[120px] rounded-full border-[5px] border-[#F8F6F4]'>
        <Image
          ...
          alt='User avatar'
        />
        <AvatarFallback>{getInitials(userHeaderData?.name || '')}</AvatarFallback>
      </Avatar>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setDialogOpen}>
        ...
        <DialogContent className='p-6'>
          ...
          <DialogDescription>Here you can edit the user details.</DialogDescription>
